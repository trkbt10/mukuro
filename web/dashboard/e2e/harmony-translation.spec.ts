...
/**
 * Harmony Translation E2E Test
 *
 * Tests the translation layer: Responses API interface -> Chat Completions internally
 * 1. Start mukuro with use_responses_api: true (but Groq provider triggers translation)
 * 2. Run multi-turn conversation with tool calls
 * 3. Verify the translation layer works correctly
 *
 * Key difference from harmony-toolcall.spec.ts:
 * - use_responses_api: true (translation mode)
 * - Backend uses Chat Completions internally but interface is Responses API
 *
 * Prerequisites:
 * - mukuro binary built: ./scripts/build-native.sh
 * - Run with: GROQ_API_KEY=<key> npx playwright test e2e/harmony-translation.spec.ts
 */
...
async function startMukuro(): Promise<void> {
  ...
  // Create config.json with groq provider using Responses API interface (with translation)
      ...
      providers: [
        {
          ...
          // KEY DIFFERENCE: use_responses_api: true triggers translation layer
          // Backend will use Chat Completions internally but interface is Responses API
...
test.describe.serial('Harmony Translation E2E', () => {
  test.skip(!GROQ_API_KEY, 'Requires GROQ_API_KEY environment variable');

  test.beforeAll(async () => {
    ctx = {
      tmpDir: fs.mkdtempSync(path.join(os.tmpdir(), 'mukuro-translation-test-')),
      process: null,
      host: 'localhost',
      port: TEST_PORT,
    };
  ...
  test('1. verify Responses API mode with translation configured', async ({ request }) => {
    ...
    // use_responses_api should be true (translation mode)
    expect(groqProvider.use_responses_api).toBe(true);
    // base_url should contain groq (triggers needs_harmony_translation)
  ...
  test('3. multi-turn conversation with tool call through translation layer', async ({ page }) => {
    // This test verifies the translation layer works for multi-turn conversations
    const result = await page.evaluate(
      async ({ host, port }) => {
        return new Promise<{
          ...
          // Multi-turn conversation to verify translation layer
          const turns = [
            // Turn 1: Trigger datetime tool call
            ...
            // Turn 3: Another follow-up to verify session state is maintained
            { sent: 'Is that a weekday or weekend?', received: null, error: null },
          ];
    ...
    // Verify all turns succeeded through translation layer
    for (let i = 0; i < result.turns.length; i++) {
      expect(result.turns[i].error, `Turn ${i + 1} error`).toBeNull();
      expect(result.turns[i].received, `Turn ${i + 1} response`).toBeTruthy();
    }
    ...
    expect(
      turn1Response.includes('time') ||
        ...
        turn1Response.includes('2026')
    ).toBe(true);

    // Turn 2 & 3: Should have valid responses (context maintained)
    expect(result.turns[1].received!.length).toBeGreaterThan(0);
    expect(result.turns[2].received!.length).toBeGreaterThan(0);
  });

  test('4. verify history records with translation layer', async () => {
    ...
    // For multi-turn, we should have multiple user/assistant pairs
    const userMessages = records.filter((r) => r.type === 'user_message');
    const assistantMessages = records.filter((r) => r.type === 'assistant_message');
    expect(userMessages.length).toBeGreaterThanOrEqual(3); // 3 turns
    expect(assistantMessages.length).toBeGreaterThanOrEqual(3); // 3 responses

    // Verify tool call records exist (datetime tool should have been called)
